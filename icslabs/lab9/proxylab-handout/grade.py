...
def start_server():
    ...
    if strace:
        ...
        '''int(subprocess.check_output("pgrep -P $(pgrep -P %d)" % (server.pid), shell=True))'''
    else:
        server.real_pid = server.pid
        '''int(subprocess.check_output("pgrep -P %d" % (server.pid), shell=True))'''
...
def start_client(port):
    ...
    try:
        ...
        raise ProxyDieException("Can't connect to your proxy aa")
    except pexpect.exceptions.TIMEOUT:
        raise ProxyDieException("Can't connect to your proxy bb")
    if strace:
        proc.real_pid = proc.pid
        '''int(subprocess.check_output("pgrep -P $(pgrep -P %d)" % (proc.pid), shell=True))'''
    else:
        proc.real_pid = proc.pid
        '''int(subprocess.check_output("pgrep -P %d" % (proc.pid), shell=True))'''
    return proc
...
def helper_reconnect(proc):
    ...
    try:
        ...
        raise ProxyDieException("Can't connect to your proxy cc")
    except pexpect.exceptions.EOF:
        raise ProxyDieException("Can't connect to your proxy dd")
