...
#	PC <- valP
#
# edit the E_bubble, D_stall, F_stall to make mrmovq rmmovq fast
#
####################################################################
...
word f_pc = [
	D_icode == IJXX && D_ifun == 1 && E_icode == IOPQ && E_ifun == 2 && E_srcA == E_srcB && E_valA > 0 : D_valP;
...
word e_valA = [    # Pass valA through stage
	M_dstM == E_srcA && E_icode == IRMMOVQ : m_valM;
	1 : E_valA;
];
...
bool F_stall =
	...
	E_icode in { IMRMOVQ, IPOPQ } &&
	 E_dstM in { d_srcA, d_srcB } && !(E_dstM == d_srcA && D_icode == IRMMOVQ && E_dstM != d_srcB )
	 ||
