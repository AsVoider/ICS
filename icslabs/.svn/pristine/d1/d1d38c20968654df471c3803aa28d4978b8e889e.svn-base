...
stat_t nexti(y64sim_t *sim)
{
    ...
    itype_t icode; //mingling leixing
    alu_t ifun;//suanshu leixing
    ...
    switch (icode)
    {
    case I_HALT: case I_NOP: case I_CALL: case I_JMP: case I_RET:
        break;
    ...
    case I_IRMOVQ: case I_RMMOVQ: case I_MRMOVQ: case I_JMP: case I_CALL:
        if(!get_long_val(sim->m, next_pc, &immdea))
        {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
    ...
    case I_RRMOVQ:/* 2:x regA:regB */
        if (!correctCond(ifun) || !NORM_REG(regA) || !NORM_REG(regB))
        {
            err_print("PC = 0x%lx, Invalid insrtuction %2.x", sim->pc, codefun);
            return STAT_INS;
        }
    ...
    case I_IRMOVQ: /* 3:0 F:regB imm */
        if (ifun != 0 || !NORM_REG(regB) || !NONE_REG(regA))
        {
            err_print("PC = 0x%lx, Invalid insrtuction %2.x", sim->pc, codefun);
            return STAT_INS;
        }
        set_reg_val(sim->r, regB, immdea);
        sim->pc = next_pc;
        break;
    case I_RMMOVQ: /* 4:0 regA:regB imm */
        if (ifun != 0 || !NORM_REG(regB) || !NORM_REG(regA))
        {
            err_print("PC = 0x%lx, Invalid insrtuction %2.x", sim->pc, codefun);
            return STAT_INS;
        }
        if (!set_long_val(sim->m, get_reg_val(sim->r, regB) + immdea, get_reg_val(sim->r, regA)))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, regB) + immdea);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
    ...
    {   long_t a = 0;
        ...
        if (!get_long_val(sim->m, get_reg_val(sim->r, regB) + immdea, &a))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, regB) + immdea);
            return STAT_ADR;
        }
        set_reg_val(sim->r, regA, a);
        sim->pc = next_pc;
        break;
    }
      case I_ALU: /* 6:x regA:regB */
	{
          if (A_NONE <= ifun || !NORM_REG(regB) || !NORM_REG(regA))
          {
              err_print("PC = 0x%lx, Invalid insrtuction %2.x", sim->pc, codefun);
              return STAT_INS;
          }
		  long_t raVal = get_reg_val(sim->r, regA), rbVal = get_reg_val(sim->r, regB);
          long_t ans = compute_alu(ifun, raVal, rbVal);
          set_reg_val(sim->r, regB, ans);
          sim->cc = compute_cc(ifun, raVal, rbVal, ans);
          sim->pc = next_pc;
          break;
	}
      case I_JMP: /* 7:x imm */
      ...
      case I_PUSHQ: /* A:0 regA:F */
          rsp = get_reg_val(sim->r, 4) - 8;
          //set_reg_val(sim->r, 4, rsp);
          if (!set_long_val(sim->m, rsp, get_reg_val(sim->r, regA)))
          {
              err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
              set_reg_val(sim->r, 4, rsp);
              return STAT_ADR;
      ...
      case I_POPQ: /* B:0 regA:F */
        {  rsp = get_reg_val(sim->r, 4) + 8;
          long_t f = 0;
          if (!get_long_val(sim->m, rsp - 8, &f))
          {
              err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
              return STAT_ADR;
          }
          set_reg_val(sim->r, 4, rsp);
          set_reg_val(sim->r, regA, f);
          sim->pc = next_pc;
          break;}
      default:
